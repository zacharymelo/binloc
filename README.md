...
**Bin Labels** (left menu) prints one label per bin at any level of the hierarchy — a tag per shelf, per bag, per rack end. Each label shows its **full location code** (identical to the system's code, the error-prevention rule), its **own level value** as a corner tag sized for reading distance, the value's description, and what is assigned below it grouped by the next level down. Contents can be switched off for identity-only labels (rack ends). **Empty bins** — configured value combinations that hold nothing yet — can be included so shelving is labelled before stock arrives; nothing is stored, the enumeration is transient.
